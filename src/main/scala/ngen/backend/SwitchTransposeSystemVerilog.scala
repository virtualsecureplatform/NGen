...
object SwitchTransposeSystemVerilog:
  ...
  private def rectangular(spec: SwitchTransposeSpec, top: String): String =
    val inBits = spec.inputLanes * spec.dataWidth
    val outBits = spec.outputLanes * spec.dataWidth
    val outputAssignments = Vector.tabulate(spec.outputLanes) { lane =>
      s"data_out[$lane*${spec.dataWidth}+:${spec.dataWidth}]<=storage[$lane*${spec.inputLanes}+output_count];"
    }.mkString(" ")
    val captureAssignments = Vector.tabulate(spec.inputLanes) { lane =>
      s"storage[input_count*${spec.inputLanes}+$lane]<=data_in[$lane*${spec.dataWidth}+:${spec.dataWidth}];"
    }.mkString(" ")
    s"""// Generated by NGen's rectangular SwitchTranspose adapter.
       ...
       |module $top(input clock,input reset,input valid_in,input [$inBits-1:0] data_in,output reg valid_out,output reg [$outBits-1:0] data_out);
       |  localparam integer INPUT_CYCLES=${spec.inputCycles},INPUT_LANES=${spec.inputLanes},OUTPUT_CYCLES=${spec.outputCycles},OUTPUT_LANES=${spec.outputLanes};
       |  reg [${spec.dataWidth - 1}:0] storage [0:${spec.inputCycles * spec.inputLanes - 1}];integer input_count,output_count,lane;
       |  always @(posedge clock) begin
       |    if(reset) begin input_count<=0;output_count<=0;valid_out<=0;data_out<='0;for(lane=0;lane<INPUT_CYCLES*INPUT_LANES;lane=lane+1)storage[lane]<='0;end
       |    else begin
       |      valid_out<=0;
       |      if(valid_in && input_count<INPUT_CYCLES) begin
       |        $captureAssignments
       |        if(input_count==INPUT_CYCLES-1) begin input_count<=INPUT_CYCLES;output_count<=0;end else input_count<=input_count+1;
       |      end else if(input_count==INPUT_CYCLES) begin
       |        valid_out<=1;$outputAssignments
       |        if(output_count==OUTPUT_CYCLES-1) begin input_count<=0;output_count<=0;end else output_count<=output_count+1;
       |      end
       |    end
       |  end
       |endmodule
  ...
  def emit(spec: SwitchTransposeSpec, top: String = "SwitchTransposeTop"): String =
    ...
    if !spec.square then rectangular(spec, top)
    else
