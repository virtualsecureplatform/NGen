...
object SwitchTransposeSystemVerilog:
  ...
  private def rectangular(spec: SwitchTransposeSpec, top: String): String =
    ...
    s"""// Generated by NGen's rectangular SwitchTranspose adapter.
       ...
       |module $top(input clock,input reset,input valid_in,output input_ready,input [$inBits-1:0] data_in,output reg valid_out,output reg [$outBits-1:0] data_out);
       ...
       |  assign input_ready=capture_active||(buffer_state[0]==FREE)||(buffer_state[1]==FREE);
       |  always @(posedge clock) begin
