...
object FusedTwiddleButterflySystemVerilog:
  def emit(modulus:Modulus,top:String="NGenFusedTwiddleButterfly"):String=
    ...
    val tileDeclarations=partials.map(_._1).mkString("\n  ")
    val productTree=partials.map(_._2).mkString("+")
    s"""$reducer
       ...
       |  localparam [$width-1:0] Q=$width'd${modulus.q};localparam [$width:0] Q_EXT=${width+1}'d${modulus.q};localparam integer DSP_TILES=${dsp.dspCount},PARTIAL_ADDER_LEVELS=${dsp.adderLevels};
       |  $tileDeclarations
