...
object RnsPolynomialMultiplierSystemVerilog:
  def emit(basis: RnsBasis, top: String = "RnsPolynomialMultiplier", emitCrt: Boolean = false): String =
    ...
    val cores = basis.domains.zipWithIndex.flatMap { case (domain,prime) =>
      Vector(
        GraphSystemVerilog.emit(GenericNttGraph.build(domain,inverse=false,profile),domain,s"${top}ForwardA$prime"),
        GraphSystemVerilog.emit(GenericNttGraph.build(domain,inverse=false,profile),domain,s"${top}ForwardB$prime"),
        GraphSystemVerilog.emit(GenericNttGraph.build(domain,inverse=true,profile),domain,s"${top}Inverse$prime")
      )
    }
    ...
    val bodies = basis.domains.zipWithIndex.map { case (domain,prime) =>
      ...
      s"""wire fa_valid_$prime,fb_valid_$prime,inv_valid_$prime;$wires
         |function automatic [${width-1}:0] rns_mul_$prime(input [${width-1}:0] a,input [${width-1}:0] b);
         ...
         |begin product={{$width{1'b0}},a}*{{$width{1'b0}},b};scaled={{${2*width}{1'b0}},product}*{{${2*width}{1'b0}},${2*width}'d${barrett.mu}};quotient=scaled[${4*width-1}:${2*width}];qp={{$width{1'b0}},quotient}*{{${2*width}{1'b0}},${width}'d${field.q}};remainder=$$signed({${width+1}'d0,product})-$$signed({1'b0,qp});if(remainder<0)remainder=remainder+${field.q};if(remainder>=${field.q})remainder=remainder-${field.q};if(remainder>=${field.q})remainder=remainder-${field.q};rns_mul_$prime=remainder[${width-1}:0];end endfunction
