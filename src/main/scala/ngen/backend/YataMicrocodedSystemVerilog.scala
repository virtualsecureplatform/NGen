...
object YataMicrocodedSystemVerilog:
  ...
  def emit(logSize: Int, streamingLog: Int, profile: ProfileName, top: String, transpose: TransposeKind = TransposeKind.Indexed): String =
    val pipelined = profile == ProfileName.F300
    val size = 1 << logSize
    ...
    val useSwitch = transpose == TransposeKind.Switch && cycles > 1
    ...
    val ports = Vector("input clock", "input reset", "input io_intt_validin") ++ Vector.tabulate(lanes)(i => s"input [31:0] io_intt_in_$i") ++
      ...
      Vector.tabulate(lanes)(i => s"input [26:0] io_ntt_in_$i") ++ Vector.tabulate(lanes)(i => s"output ${if pipelined then "" else "reg "}[31:0] io_ntt_out_$i") ++ Vector(s"output ${if pipelined then "" else "reg "}io_ntt_validout")
    ...
    val modswitchInstances = Vector.tabulate(lanes)(i =>
      if pipelined then s"wire torus_valid_$i;YataModSwitchPipeline modswitch_$i(clock,reset,output_ntt,torus_input_$i,torus_valid_$i,io_ntt_out_$i);" + (if i == 0 then "assign io_ntt_validout=torus_valid_0;" else "")
      else s"YataModSwitch modswitch_$i(torus_input_$i,torus$i);")
    ...
    def inputCases(isIntt: Boolean): String = (0 until cycles).map { cycle =>
      val assignments = Vector.tabulate(lanes) { lane =>
        val index = if isIntt && !useSwitch then lane * cycles + cycle else cycle * lanes + lane
    ...
    def outputCases(isIntt: Boolean): String = (0 until cycles).map { cycle =>
      val assignments = Vector.tabulate(lanes) { lane =>
        ...
        if isIntt then s"io_intt_out_$lane<=${w(index)}[26:0];" else if pipelined then "" else s"io_ntt_out_$lane<=torus$lane;"
      }
      s"$cycle: begin\n${lines(assignments,10)}\n        end"
    }.mkString("\n")
    val counterDeclarations = if pipelined then
      s"reg [${32-Integer.numberOfLeadingZeros(inverse.size.max(forward.size)-1)-1}:0] pc;reg [${math.max(1,Integer.numberOfTrailingZeros(cycles))-1}:0] input_count,output_count;reg [2:0] stall_count;"
    else "integer pc,input_count,output_count,stall_count;"
    val core = s"""// Generated by NGen's microcoded YATA radix-8 backend.
       ...
       |  $counterDeclarations reg executing,inverse_operation,finishing,output_intt,output_ntt;
       |  $modswitchMux
       ...
       |""".stripMargin
    val pipelinedCore = if pipelined then
      core.replace("io_ntt_validout<=0;", "").replace("io_ntt_validout<=1;", "") + "\n" + YataMicroLanePipeline.definition + "\n" + YataMicroLanePipeline.outputConversion
    else core
  ...
  private def switchWrapper(top: String, coreName: String, lanes: Int, cycles: Int): String =
    ...
    val groups = lanes / cycles
    val spec = SwitchTransposeSpec(bits, 32)
    val ports = Vector("input clock", "input reset", "input io_intt_validin") ++ Vector.tabulate(lanes)(i => s"input [31:0] io_intt_in_$i") ++
    ...
    val coreInttInputs = Vector.tabulate(lanes)(i => s"wire [31:0] core_intt_in_$i;")
    val coreInttOutputs = Vector.tabulate(lanes)(i => s"wire [26:0] core_intt_out_$i; assign io_intt_out_$i=core_intt_out_$i;")
    val coreNttOutputs = Vector.tabulate(lanes)(i => s"wire [31:0] core_ntt_out_$i;")
    val needsRegroup = groups > 1
    val bufferDeclarations = if needsRegroup then Vector.tabulate(lanes * cycles)(i => s"reg [31:0] switch_buffer_$i;") else Vector.empty
    val feedDeclarations = if needsRegroup then Vector.tabulate(groups)(g => s"reg [${cycles*32-1}:0] ntt_feed_$g;") else Vector.empty
    val captureCases = if needsRegroup then (0 until cycles).map { cycle =>
      val assignments = Vector.tabulate(lanes)(lane => s"switch_buffer_${cycle*lanes+lane}<=core_ntt_out_$lane;")
      s"$cycle:begin ${assignments.mkString(" ")} end"
    }.mkString("\n") else ""
    val feedCases = if needsRegroup then (0 until cycles).map { feedCycle =>
      val assignments = for
        group <- 0 until groups
        lane <- 0 until cycles
      yield s"ntt_feed_$group[${lane*32}+:32]=switch_buffer_${group*cycles*cycles+feedCycle*cycles+lane};"
      s"$feedCycle:begin ${assignments.mkString(" ")} end"
    }.mkString("\n") else ""
    val groupWiring = Vector.tabulate(groups) { group =>
      val widthBits = cycles * 32
      val inttInputs = Vector.tabulate(cycles)(lane => s"assign intt_group_${group}_in[${lane*32}+:32]=io_intt_in_${lane*groups+group};")
      val inttOutputs = Vector.tabulate(cycles)(lane => s"assign core_intt_in_${group*cycles+lane}=intt_group_${group}_out[${lane*32}+:32];")
      val nttInputs =
        if needsRegroup then Vector(s"assign ntt_group_${group}_in=ntt_feed_$group;")
        else Vector.tabulate(cycles)(lane => s"assign ntt_group_${group}_in[${lane*32}+:32]=core_ntt_out_${lane*groups+group};")
      val nttOutputs = Vector.tabulate(cycles)(lane => s"assign io_ntt_out_${group*cycles+lane}=ntt_group_${group}_out[${lane*32}+:32];")
      s"""wire [$widthBits-1:0] intt_group_${group}_in,intt_group_${group}_out,ntt_group_${group}_in,ntt_group_${group}_out; wire intt_group_${group}_valid,ntt_group_${group}_valid;
         |${(inttInputs++inttOutputs++nttInputs++nttOutputs).mkString("\n")}
         |NGenSwitchTransposeNetwork_$bits intt_switch_$group(clock,reset,io_intt_validin,intt_group_${group}_in,intt_group_${group}_valid,intt_group_${group}_out);
         |NGenSwitchTransposeNetwork_$bits ntt_switch_$group(clock,reset,${if needsRegroup then "feed_active" else "core_ntt_validout"},ntt_group_${group}_in,ntt_group_${group}_valid,ntt_group_${group}_out);""".stripMargin
    }
    val coreConnections = Vector(".clock(clock)", ".reset(reset)", ".io_intt_validin(intt_group_0_valid)") ++
      Vector.tabulate(lanes)(i => s".io_intt_in_$i(core_intt_in_$i)") ++ Vector.tabulate(lanes)(i => s".io_intt_out_$i(core_intt_out_$i)") ++
      Vector(".io_intt_validout(io_intt_validout)", ".io_ntt_validin(io_ntt_validin)") ++ Vector.tabulate(lanes)(i => s".io_ntt_in_$i(io_ntt_in_$i)") ++
      Vector.tabulate(lanes)(i => s".io_ntt_out_$i(core_ntt_out_$i)") ++ Vector(".io_ntt_validout(core_ntt_validout)")
    s"""${SwitchTransposeSystemVerilog.definitions(spec)}
       |module $top(
       |${ports.map("  "+_).mkString(",\n")}
       |);
       |wire core_ntt_validout;
       |${(coreInttInputs++coreInttOutputs++coreNttOutputs++bufferDeclarations++feedDeclarations).mkString("\n")}
       |${if needsRegroup then s"integer capture_count,feed_count;reg feed_active;always @(*)begin ${Vector.tabulate(groups)(g=>s"ntt_feed_$g=0;").mkString(" ")} case(feed_count) $feedCases endcase end always @(posedge clock)begin if(reset)begin capture_count<=0;feed_count<=0;feed_active<=0;end else begin if(core_ntt_validout)begin case(capture_count) $captureCases endcase if(capture_count==$cycles-1)begin capture_count<=0;feed_count<=0;feed_active<=1;end else capture_count<=capture_count+1;end if(feed_active)begin if(feed_count==$cycles-1)begin feed_count<=0;feed_active<=0;end else feed_count<=feed_count+1;end end end" else ""}
       |${groupWiring.mkString("\n")}
       |assign io_ntt_validout=ntt_group_0_valid;
       |$coreName core(${coreConnections.mkString(",")});
       |endmodule
       |""".stripMargin
