  ...
  def emit(graph: TimedGraph, field: Modulus, size: Int, top: String,reduction:ReductionKind): String =
    ...
    def expression(node: Node): String = node.operator match
      ...
      case op: BarrettMultiplyConstant =>
        s"field_mul(${reference(node.inputs.head)}, ${width}'d${field.normalize(op.constant)})"
    ...
    val multiplyFunction=if reduction==ReductionKind.SparseFold then SparseFoldFunction.emit(field)
    else s"""function automatic [${width - 1}:0] field_mul(input [${width - 1}:0] a, input [${width - 1}:0] b);
